...
// 创建wakeupfd，用来唤醒subRector处理新来的channel
int createEventfd()
{
...
void EventLoop::runInloop(Functor cb)
{
    ...
    else queueInloop(cb);   // 不是自己线程的loop，就要唤醒对应的loop，让对应的loop去执行回调
}
...
void EventLoop::handleRead()
{
    ...
    if (n != sizeof one) LOG_ERROR("EventLoop::handleRead() reads %lu bytes instead of 8", n);
}
...
void EventLoop::wakeup()
{
    ...
    if(n != sizeof one) LOG_ERROR("EventLoop::wakeup() writes %lu bytes instead of 8 \n", n);
}
