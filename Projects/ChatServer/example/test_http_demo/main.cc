...
int main() {
    ...
    HttpServer server(8080, "TestHttpServer", true);
    server.setSessionManager(std::make_unique<session::SessionManager>(std::move(sessionManager)));

    ssl::SslConfig sslConfig;
    sslConfig.setCertFile("/home/ddy/Projects/AiApp/example/test_http_demo/server.crt");
    sslConfig.setKeyFile("/home/ddy/Projects/AiApp/example/test_http_demo/server.key");
